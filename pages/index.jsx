    ...
    <>
      ...
      <div style={{ display: 'flex', height: '100vh', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: C.white, overflow: 'hidden' }}>
          ...
          {showEmoji && (
            <div style={{ padding: '8px 18px', background: C.bg, borderTop: '1px solid ' + C.border, display: 'flex', flexWrap: 'wrap', gap: 4, flexShrink: 0, maxHeight: 120, overflowY: 'auto' }}>
              {['😀','😂','🥰','😍','😊','😉','😎','🤔','😢','😭','😡','🥳','😴','🤗','🙄','😬','👍','👎','🙏','👋','💪','✌️','🤞','👏','❤️','🧡','💛','💚','💙','💜','🖤','🤍','💔','💕','✨','🔥','⭐','🎉','🎂','☕','🐱','🐶','🦋','🌙','☀️','🌈','💭','💡','✅','❌'].map(function(e) {
                return <button key={e} onClick={function() { setInput(function(prev) { return prev + e; }); }} style={{ fontSize: 20, background: 'none', padding: 4, lineHeight: 1 }}>{e}</button>;
              })}
            </div>
          )}
